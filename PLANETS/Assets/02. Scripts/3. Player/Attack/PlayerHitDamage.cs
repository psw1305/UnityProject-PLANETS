...
public class PlayerHitDamage : MonoBehaviour 
{
    ...
    void BulletAiming() 
	{
        if (FindClosestPlayer() != null && chase)
            destination = FindClosestPlayer().transform.position;
        else if (FindClosestPlayer() == null && chase)
            Explosion();
    ...
    void BulletDisable()
	{
        if (host != null)
        {
            ...
            if (distance2 >= hostRadius && hostRadius != 0)
